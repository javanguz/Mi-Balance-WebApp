...
module.exports = function(requireLicense) {
    ...
    function drawPdfResumenRow(doc, items, level, config) {
        ...
        items.forEach(item => {
            ...
            if (isAllTypes) {
                ...
                const amount = tipo === 'ingreso' ? 
                item.ingresos : -item.egresos; 
    ...
    async function getReportData(queryParams) {
        ...
        let sql = `
            ...
            FROM movimientos m 
            LEFT JOIN clientes c ON m.entidad_id = c.id AND 
    ...
    function generarPdfDetalle(res, data, username) {
        ...
        data.movimientos.forEach(m => {
            ...
            row.forEach((cell, i) => { 
                doc.text(cell, x, y, { width: colWidths[i], align: i === 4 ? 
                'right' : 'left' }); 
                x += colWidths[i];
            });
    ...
    function generarPdfResumen(res, data, username) {
        ...
        const headers = isAllTypes ? 
        ['Concepto', 'Ingresos', 'Egresos', 'Resultado'] : ['Concepto', 'Importe']; 
        ...
        if (isAllTypes) {
            ...
            const totalAmount = data.tipo === 'ingreso' ? 
            data.totalIngresos : -data.totalEgresos; 
    ...
    router.get('/exportar/detalle/:formato', requireLicense, async (req, res, next) => {
        try {
            ...
            if (formato === 'pdf') generarPdfDetalle(res, data, res.locals.username);
    ...
    router.get('/exportar/resumen/:formato', requireLicense, async (req, res, next) => {
        try {
            ...
            if (formato === 'pdf') generarPdfResumen(res, data, res.locals.username);
